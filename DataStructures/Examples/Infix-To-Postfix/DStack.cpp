 ...
 * 
 * This is the implementation file for all Dynamic Stack functions
******************************************************************/
...
Stack::Stack(const Stack& original)                                 // Definition of Stack copy constructor
{
    ...
    myArray = new(nothrow) StackElement[myCapacity];                // Get new array for copy
                                                                    // allocate array in copy
    if (myArray != 0)                                               // check if memory available
        {
            for (int pos = 0; pos < myCapacity; pos++)              // copy original's array member into this new array
...
Description: Copys an array into a new array.
...
/************************************************************
Function Name: empty()
Decription: Check if stack is empty.
Precondition: None
...
StackElement Stack::top() const                                     // Definition of top()
{
    if ( !empty() )
    ...
    else
        {
            ...
            cerr << "*** returning garbage value ***" << endl;
